...
def add_wavs(path):
    ...
    for row in csv_reader:
        ...
        soundfile.write('./new/audios/{0}'.format(name), data, 16000)
	    	
        file_name = os.path.abspath('./new/audios/{0}'.format(name))
        file_size = os.stat('./new/audios/{0}'.format(name)).st_size
...
def generate_sets(wav_list, gender):
	...
	with open('./new/train/train_'+gender+'.csv', 'w') as myfile:
	    wr = csv.writer(myfile)
	    wr.writerow(['wav_filename','wav_filesize','transcript'])
	    for row in train:
	    	wr.writerow(row)

	with open('./new/dev/dev_'+gender+'.csv', 'w') as myfile:
	    wr = csv.writer(myfile)
	    wr.writerow(['wav_filename','wav_filesize','transcript'])
	    for row in dev:
	    	wr.writerow(row)

	with open('./new/test/test_'+gender+'.csv', 'w') as myfile:
	    wr = csv.writer(myfile)
	    wr.writerow(['wav_filename','wav_filesize','transcript'])
	    for row in test:
	    	wr.writerow(row)
...
def preprocess_slr_dataset():
    ...
    male_wav_list = add_wavs('/mcv_cl_male.tsv')
    generate_sets(male_wav_list,'male')
    print("Preprocesando data mujeres")
    female_wav_list= add_wavs('/mcv_cl_female.tsv')
